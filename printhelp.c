...
void printhelp()
{
	...
	printf("    Memory addresses are byte addressed.  Suffix of i=instruction or d=data memory\n");
	...
	printf("    Raw dump of PRU data memory (byte offset from beginning of full PRU memory block - all PRUs)\n\n");

	printf("    DD <address> [length]\n");
	printf("    Dump data memory (byte offset from beginning of PRU data memory)\n\n");

	printf("    DI <address> [length]\n");
	printf("    Dump instruction memory (byte offset from beginning of PRU instruction memory)\n\n");
	...
	printf("      format 4:  wa watch_num address len - set a watch point (watch_num) so any change at that byte address\n");
	printf("                 for <len> bytes in data memory will be printed during program execution with gss command\n");
	printf("      format 5:  wa watch_num address : value0 value1 ... - set a watch point (watch_num) so that the program\n");
	printf("                 (run with gss) will be halted when the memory span at that location location equals the values\n");
	printf("                 specified\n");
	printf("      NOTE: for watchpoints to work, you must use gss command to run the program\n\n");

	printf("    WR <address> value1 [value2 [value3 ...]]\n");
	printf("    Write a byte value to a raw (offset from beginning of full PRU memory block - all PRUs)\n");
	printf("    memory_location is a byte address from the beginning of the PRU subsystem memory block\n\n");

	printf("    WRD <address> value1 [value2 [value3 ...]]\n");
	printf("    Write a byte value to PRU data memory (byte offset from beginning of PRU data memory)\n\n");

	printf("    WRI <address> value1 [value2 [value3 ...]]\n");
	printf("    Write a byte value to PRU instruction memory (byte offset from beginning of PRU instruction memory)\n\n");
